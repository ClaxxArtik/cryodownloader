...
def download_audio(url):
    ...
    audio_dir = os.path.join("downloads", "audio") # Set the directory for audio downloads
    os.makedirs(audio_dir, exist_ok=True) # Create the directory if it doesn't exist

    ydl_audio = {
    'format': 'mp3/bestaudio/best',
    'outtmpl': os.path.join(audio_dir, '%(title)s.%(ext)s'), # Save to downloads/audio folder with title as filename
...
def download_video(url):
    ...
    video_dir = os.path.join("downloads", "video") # Set the directory for video downloads
    os.makedirs(video_dir, exist_ok=True) # Create the directory if it doesn't exist

    ydl_video = {
    'format': 'bestvideo[height<=1080]+bestaudio/best',
    'outtmpl': os.path.join(video_dir, '%(title)s.%(ext)s'), # Save to downloads/video folder with title as filename
    'postprocessors': [{  # Extract audio using ffmpeg
